...
void Player::update(float deltaT) {
  ...
  if (gravity) {
    ...
    if (swimming)
      gravityVector.y = std::max(gravityVector.y, -1.5f);

    auto collisionResponse =
        Movement::resolveCollision(currentPosition, dY, chunkMap);
  ...
  if (collision) {
    static const std::vector<glm::vec3> axes = {glm::vec3(1, 0, 0),
                                                glm::vec3(0, 0, 1)};

    movement = glm::vec3(glm::rotate(glm::mat4(1.0f), currentAngle.y,
                                     glm::vec3(0.0f, 1.0f, 0.0f)) *
                         glm::vec4(movement.x, movement.y, movement.z, 1));

    auto collisionResponse =
        Movement::resolveCollision(currentPosition, movement, chunkMap);
    currentPosition = collisionResponse.position;
    camera.setPosition(currentPosition);
  } else {
    finalMovement = glm::vec3(glm::rotate(glm::mat4(1.0f), currentAngle.y,
                                          glm::vec3(0.0f, 1.0f, 0.0f)) *
                              glm::vec4(movement.x, movement.y, movement.z, 1));
    ;
