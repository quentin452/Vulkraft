...
#include "stb_image.h"
void VulkraftApplication::run() {
  Globals::chunkDrawing.clearBuffers();
  initWindow();
  initVulkan();
  mainLoop();
  cleanup();
}
void VulkraftApplication::updateVertexBuffer() {
  ...
  createBuffer(bufferSize,
               ...
               VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, vertexBuffer[curBuffer],
               vertexBufferMemory[curBuffer]);
  ...
  createBuffer(
      ...
      VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, waterVertexBuffer[curBuffer],
      waterVertexBufferMemory[curBuffer]);
...
void VulkraftApplication::updateIndexBuffer() {
  PROFILE_SCOPED(std::string("Vulkraft:") + ":" + __FUNCTION__)
  ...
  createBuffer(bufferSize,
               ...
               VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, indexBuffer[curBuffer],
               indexBufferMemory[curBuffer]);
  ...
  createBuffer(bufferSize,
               VK_BUFFER_USAGE_TRANSFER_DST_BIT |
                   VK_BUFFER_USAGE_INDEX_BUFFER_BIT,
               VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, waterIndexBuffer[curBuffer],
               waterIndexBufferMemory[curBuffer]);
...
void VulkraftApplication::initWindow() {
  glfwInit();

  glfwWindowHint(GLFW_CLIENT_API, GLFW_NO_API);

  window =
      glfwCreateWindow(WIDTH, HEIGHT, Globals::GAME_TITLE, nullptr, nullptr);
  glfwSetWindowUserPointer(window, this);
  glfwSetFramebufferSizeCallback(window, framebufferResizeCallback);
}

void VulkraftApplication::framebufferResizeCallback(GLFWwindow *window,
                                                    int width, int height) {
  auto app =
      reinterpret_cast<VulkraftApplication *>(glfwGetWindowUserPointer(window));
  app->framebufferResized = true;
}

void VulkraftApplication::initVulkan() {
  createInstance();
  setupDebugMessenger();
  createSurface();
  pickPhysicalDevice();
  ...
  Globals::chunkDrawing.initChunks();
  createVertexBuffer();
  createIndexBuffer();
...
void VulkraftApplication::createInstance() {
  PROFILE_SCOPED(std::string("Vulkraft:") + ":" + __FUNCTION__)
  ...
  appInfo.pApplicationName = Globals::GAME_TITLE;
...
std::vector<const char *> VulkraftApplication::getRequiredExtensions() {
  PROFILE_SCOPED(std::string("Vulkraft:") + ":" + __FUNCTION__)
...
bool VulkraftApplication::checkValidationLayerSupport() {
  PROFILE_SCOPED(std::string("Vulkraft:") + ":" + __FUNCTION__)
  uint32_t layerCount;
...
void VulkraftApplication::populateDebugMessengerCreateInfo(
  ...
  createInfo.pfnUserCallback = &VulkraftApplication::debugCallback;
}

VKAPI_ATTR VkBool32 VKAPI_CALL VulkraftApplication::debugCallback(
...
VkResult VulkraftApplication::CreateDebugUtilsMessengerEXT(
...
void VulkraftApplication::setupDebugMessenger() {
  PROFILE_SCOPED(std::string("Vulkraft:") + ":" + __FUNCTION__)
  if (!enableValidationLayers)
    return;
...
void VulkraftApplication::createSurface() {
  PROFILE_SCOPED(std::string("Vulkraft:") + ":" + __FUNCTION__)
...
void VulkraftApplication::pickPhysicalDevice() {
  PROFILE_SCOPED(std::string("Vulkraft:") + ":" + __FUNCTION__)
...
bool VulkraftApplication::isDeviceSuitable(VkPhysicalDevice device) {
  PROFILE_SCOPED(std::string("Vulkraft:") + ":" + __FUNCTION__)
...
QueueFamilyIndices
VulkraftApplication::findQueueFamilies(VkPhysicalDevice device) {
  PROFILE_SCOPED(std::string("Vulkraft:") + ":" + __FUNCTION__)
...
bool VulkraftApplication::checkDeviceExtensionSupport(VkPhysicalDevice device) {
  PROFILE_SCOPED(std::string("Vulkraft:") + ":" + __FUNCTION__)
...
VulkraftApplication::querySwapChainSupport(VkPhysicalDevice device) {
  PROFILE_SCOPED(std::string("Vulkraft:") + ":" + __FUNCTION__)
...
VkSampleCountFlagBits VulkraftApplication::getMaxUsableSampleCount() {
  PROFILE_SCOPED(std::string("Vulkraft:") + ":" + __FUNCTION__)
...
void VulkraftApplication::createLogicalDevice() {
  PROFILE_SCOPED(std::string("Vulkraft:") + ":" + __FUNCTION__)
...
void VulkraftApplication::createSwapChain() {
  PROFILE_SCOPED(std::string("Vulkraft:") + ":" + __FUNCTION__)
...
VkSurfaceFormatKHR VulkraftApplication::chooseSwapSurfaceFormat(
    const std::vector<VkSurfaceFormatKHR> &availableFormats) {
  PROFILE_SCOPED(std::string("Vulkraft:") + ":" + __FUNCTION__)

  // Cherche un format compatible avec les besoins de la swapchain
  ...
  // Utilise un format par défaut si aucun format spécifique n'est trouvé
  return availableFormats[0];
}

VkPresentModeKHR VulkraftApplication::chooseSwapPresentMode(
...
VkExtent2D VulkraftApplication::chooseSwapExtent(
...
void VulkraftApplication::createImageViews() {
  PROFILE_SCOPED(std::string("Vulkraft:") + ":" + __FUNCTION__)
...
VkImageView VulkraftApplication::createImageView(VkImage image, VkFormat format,
...
void VulkraftApplication::createRenderPass() {
  PROFILE_SCOPED(std::string("Vulkraft:") + ":" + __FUNCTION__)
...
void VulkraftApplication::createDescriptorSetLayouts() {
  PROFILE_SCOPED(std::string("Vulkraft:") + ":" + __FUNCTION__)
  createDescriptorSetLayout();
}

void VulkraftApplication::createDescriptorSetLayout() {
  PROFILE_SCOPED(std::string("Vulkraft:") + ":" + __FUNCTION__)
  ...
  std::array<VkDescriptorSetLayoutBinding, 3> bindings = {
      vertexUboLayoutBinding, fragmentUboLayoutBinding, samplerLayoutBinding};
...
void VulkraftApplication::createPipelines() {
  PROFILE_SCOPED(std::string("Vulkraft:") + ":" + __FUNCTION__)
  createGraphicsPipeline();
}

void VulkraftApplication::createGraphicsPipeline() {
  PROFILE_SCOPED(std::string("Vulkraft:") + ":" + __FUNCTION__)
...
std::vector<char> VulkraftApplication::readFile(const std::string &filename) {
  PROFILE_SCOPED(std::string("Vulkraft:") + ":" + __FUNCTION__)
...
VkShaderModule
VulkraftApplication::createShaderModule(const std::vector<char> &code) {
  PROFILE_SCOPED(std::string("Vulkraft:") + ":" + __FUNCTION__)
...
void VulkraftApplication::createFramebuffers() {
  PROFILE_SCOPED(std::string("Vulkraft:") + ":" + __FUNCTION__)
...
void VulkraftApplication::createCommandPool() {
  PROFILE_SCOPED(std::string("Vulkraft:") + ":" + __FUNCTION__)
...
void VulkraftApplication::createColorResources() {
  PROFILE_SCOPED(std::string("Vulkraft:") + ":" + __FUNCTION__)
...
void VulkraftApplication::createDepthResources() {
  PROFILE_SCOPED(std::string("Vulkraft:") + ":" + __FUNCTION__)
...
VkFormat VulkraftApplication::findDepthFormat() {
  PROFILE_SCOPED(std::string("Vulkraft:") + ":" + __FUNCTION__)
...
VkFormat VulkraftApplication::findSupportedFormat(
...
bool VulkraftApplication::hasStencilComponent(VkFormat format) {
  PROFILE_SCOPED(std::string("Vulkraft:") + ":" + __FUNCTION__)
  return format == VK_FORMAT_D32_SFLOAT_S8_UINT ||
         format == VK_FORMAT_D24_UNORM_S8_UINT;
}

void VulkraftApplication::createTextureImage() {
  PROFILE_SCOPED(std::string("Vulkraft:") + ":" + __FUNCTION__)
...
void VulkraftApplication::generateMipmaps(VkImage image, VkFormat imageFormat,
...
void VulkraftApplication::createImage(
...
void VulkraftApplication::transitionImageLayout(VkImage image, VkFormat format,
...
void VulkraftApplication::copyBufferToImage(VkBuffer buffer, VkImage image,
...
void VulkraftApplication::createTextureImageView() {
  PROFILE_SCOPED(std::string("Vulkraft:") + ":" + __FUNCTION__)
  textureImageView = createImageView(textureImage, VK_FORMAT_R8G8B8A8_SRGB,
                                     VK_IMAGE_ASPECT_COLOR_BIT, mipLevels);
}

void VulkraftApplication::createTextureSampler() {
  PROFILE_SCOPED(std::string("Vulkraft:") + ":" + __FUNCTION__)
...
void VulkraftApplication::createVertexBuffer() {
  PROFILE_SCOPED(std::string("Vulkraft:") + ":" + __FUNCTION__)
  VkDeviceSize bufferSize = sizeof(BlockVertex) * vertices.size();
  ...
  for (int i = 0; i < MAX_FRAMES_IN_FLIGHT; ++i) {
    ...
    createBuffer(bufferSize,
                 VK_BUFFER_USAGE_TRANSFER_DST_BIT |
                     VK_BUFFER_USAGE_VERTEX_BUFFER_BIT,
                 VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, vertexBuffer[i],
  ...
  for (int i = 0; i < MAX_FRAMES_IN_FLIGHT; ++i) {
    ...
    createBuffer(bufferSize,
                 VK_BUFFER_USAGE_TRANSFER_DST_BIT |
                     VK_BUFFER_USAGE_VERTEX_BUFFER_BIT,
                 VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, waterVertexBuffer[i],
                 waterVertexBufferMemory[i]);
...
void VulkraftApplication::createBuffer(VkDeviceSize size,
...
uint32_t VulkraftApplication::findMemoryType(uint32_t typeFilter,
                                             VkMemoryPropertyFlags properties) {
  PROFILE_SCOPED(std::string("Vulkraft:") + ":" + __FUNCTION__)
...
void VulkraftApplication::copyBuffer(VkBuffer srcBuffer, VkBuffer dstBuffer,
                                     VkDeviceSize size) {
  PROFILE_SCOPED(std::string("Vulkraft:") + ":" + __FUNCTION__)
  VkCommandBuffer commandBuffer = beginSingleTimeCommands();
...
VkCommandBuffer VulkraftApplication::beginSingleTimeCommands() {
  PROFILE_SCOPED(std::string("Vulkraft:") + ":" + __FUNCTION__)
...
void VulkraftApplication::endSingleTimeCommands(VkCommandBuffer commandBuffer) {
  PROFILE_SCOPED(std::string("Vulkraft:") + ":" + __FUNCTION__)
...
void VulkraftApplication::createIndexBuffer() {
  PROFILE_SCOPED(std::string("Vulkraft:") + ":" + __FUNCTION__)
  VkDeviceSize bufferSize = sizeof(uint32_t) * indices.size();
  ...
  for (int i = 0; i < MAX_FRAMES_IN_FLIGHT; ++i) {
    ...
    createBuffer(bufferSize,
                 VK_BUFFER_USAGE_TRANSFER_DST_BIT |
                     VK_BUFFER_USAGE_INDEX_BUFFER_BIT,
                 VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, indexBuffer[i],
                 indexBufferMemory[i]);
  ...
  for (int i = 0; i < MAX_FRAMES_IN_FLIGHT; ++i) {
    ...
    createBuffer(bufferSize,
                 VK_BUFFER_USAGE_TRANSFER_DST_BIT |
                     VK_BUFFER_USAGE_INDEX_BUFFER_BIT,
                 VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, waterIndexBuffer[i],
                 waterIndexBufferMemory[i]);
...
void VulkraftApplication::createUniformBuffers() {
  PROFILE_SCOPED(std::string("Vulkraft:") + ":" + __FUNCTION__)
  ...
  for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
    createBuffer(vertexBufferSize, VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
                 VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT |
                     VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
                 vertexUniformBuffers[i], vertexUniformBuffersMemory[i]);
    createBuffer(fragmentBufferSize, VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
                 VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT |
                     VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
                 fragmentUniformBuffers[i], fragmentUniformBuffersMemory[i]);
  }
}

void VulkraftApplication::createDescriptorPool() {
  PROFILE_SCOPED(std::string("Vulkraft:") + ":" + __FUNCTION__)
...
void VulkraftApplication::createDescriptorSets() {
  PROFILE_SCOPED(std::string("Vulkraft:") + ":" + __FUNCTION__)
  createDescriptorSet();
}

void VulkraftApplication::createDescriptorSet() {
  PROFILE_SCOPED(std::string("Vulkraft:") + ":" + __FUNCTION__)
...
void VulkraftApplication::createCommandBuffers() {
  PROFILE_SCOPED(std::string("Vulkraft:") + ":" + __FUNCTION__)
...
void VulkraftApplication::createSyncObjects() {
  PROFILE_SCOPED(std::string("Vulkraft:") + ":" + __FUNCTION__)
...
void VulkraftApplication::mainLoop() {
  PROFILE_SCOPED(std::string("Vulkraft:") + ":" + __FUNCTION__)
  setTimeNoon();
  toggleCursor();
...
void VulkraftApplication::drawFrame() {
  PROFILE_SCOPED(std::string("Vulkraft:") + ":" + __FUNCTION__)
...
void VulkraftApplication::updateUniformBuffer(uint32_t currentImage) {
  PROFILE_SCOPED(std::string("Vulkraft:") + ":" + __FUNCTION__)
  ...
  if (glfwGetKey(window, GLFW_KEY_P)) {
    if (time - debounce > 0.33) {
      Globals::chunkDrawing.player.updatePhysics();
      debounce = time;
    }
  }
  ...
  Globals::chunkDrawing.updateUniform(deltaT, vertices, curBuffer);

  // Update sun (and moon) position, visibility is a [0,1] value used to
  // switch the sun (moon) off when it's below the horizon (1 - visibility is
  // the equivalent for the moon)
  const float SUN_SPEED = glm::radians(0.03f);
  const float threshold = 0.75f;
  sunDir = (glm::rotate(glm::mat4(1.0f), SUN_SPEED, glm::vec3(1, 0, 0)) *
            glm::vec4(sunDir, 1.0f));
  ...
  // View matrix
  glm::mat4 View = Globals::chunkDrawing.player.getCamera().getMatrix();
  ...
  fubo.eyePos = Globals::chunkDrawing.player.getCamera().getPosition();
  fubo.eyeDir = Globals::chunkDrawing.player.getCamera().getDirection();
  fubo.ambientParams.x = visibility * 0.175 + (1 - visibility) * 0.025;
  fubo.ambientParams.y = Globals::chunkDrawing.player.isSwimming();

  void *data;
  vkMapMemory(device, vertexUniformBuffersMemory[currentImage], 0, sizeof(vubo),
              0, &data);
...
void VulkraftApplication::recordCommandBuffer(VkCommandBuffer commandBuffer,
                                              uint32_t imageIndex,
                                              int currentFrame) {
  PROFILE_SCOPED(std::string("Vulkraft:") + ":" + __FUNCTION__)
  VkCommandBufferBeginInfo beginInfo{};
  ...
  vkCmdBindIndexBuffer(commandBuffer, indexBuffer[curBuffer], 0,
  ...
  vkCmdBindIndexBuffer(commandBuffer, waterIndexBuffer[curBuffer], 0,
                       VK_INDEX_TYPE_UINT32);

  vkCmdDrawIndexed(commandBuffer, static_cast<uint32_t>(waterIndices.size()), 1,
                   0, 0, 0);
...
void VulkraftApplication::recreateSwapChain() {
  PROFILE_SCOPED(std::string("Vulkraft:") + ":" + __FUNCTION__)
...
void VulkraftApplication::cleanupSwapChain() {
  PROFILE_SCOPED(std::string("Vulkraft:") + ":" + __FUNCTION__)
...
void VulkraftApplication::cleanup() {
  PROFILE_SCOPED(std::string("Vulkraft:") + ":" + __FUNCTION__)
  cleanupSwapChain();
